 ...
 *
 * We pull the images and a plain-text summary, and let the screen render
 * those separately rather than dumping the raw HTML (which would carry
 * untrusted markup directly into the UI).
 *
 * Ported verbatim from the web app (lib/spotmap/parseKmlDescription.ts) —
 * pure JS, no DOM, safe on React Native.
 */
...
export function parseKmlDescription(
  html: string | null | undefined,
): ParsedKmlDescription {
