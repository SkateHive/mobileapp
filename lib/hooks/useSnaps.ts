...
import { getSnapsContainers, getContentReplies, ExtendedComment, SNAPS_CONTAINER_AUTHOR, SNAPS_PAGE_MIN_SIZE, COMMUNITY_TAG } from '../hive-utils';
    ...
    while (allFilteredComments.length < pageSize && hasMoreData && iterationCount < maxIterations) {
      ...
      try {
        ...
        for (const resultItem of result) {
          ...
          // Add filtered comments to results
          ...
          // Break early if we have enough posts for this iteration
          if (allFilteredComments.length >= pageSize) {
            break;
          }
        }
      } catch (error) {
