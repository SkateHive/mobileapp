...
export const NotificationItem = React.memo(
  ({ notification }: NotificationItemProps) => {
    const [isConversationDrawerVisible, setIsConversationDrawerVisible] =
      useState(false);
    ...
    const getNotificationType = (
      ...
      if (
        msg.includes("mentioned you") ||
        msg.includes("mentioned") ||
        type === "mention"
      ) {
        return "mention";
      }
      ...
      // Check for follows
      if (
        msg.includes("started following") ||
        msg.includes("followed") ||
        type === "follow"
      ) {
        return "follow";
      }
      ...
      try {
        if (notificationType === "reply" || notificationType === "mention") {
          ...
          if (postInfo) {
            ...
            if (
              replyContent &&
              replyContent.parent_author &&
              replyContent.parent_permlink
            ) {
              // Fetch the parent post
              const parentContent = await getContent(
                replyContent.parent_author,
                replyContent.parent_permlink,
              );
              if (parentContent) {
      ...
      <>
        ...
        >
          ...
          <View style={styles.avatarContainer}>
            <Image
              source={{
                uri: `${HIVE_AVATAR_URL}/${author}/avatar/small`,
              }}
              style={styles.avatar}
            />
          </View>
